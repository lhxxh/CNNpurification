...
for trial in range(num_trials):
  ...
  zero_index_idx = choice(len(zero_index),(batch//3)*2,replace=False)
  one_index_idx = choice(len(one_index),(batch//3)*2,replace=False)
  two_index_idx = choice(len(two_index),(batch//3)*2,replace=False)
  chosen_zero = dataset_images[zero_index[zero_index_idx[:batch//3]]]
  chosen_one = dataset_images[one_index[one_index_idx[:batch//3]]]
  chosen_two = dataset_images[two_index[two_index_idx[:batch//3]]]
  ...
  chosen_one = dataset_images[one_index[one_index_idx[batch//3:]]]
  chosen_two = dataset_images[two_index[two_index_idx[batch//3:]]]
  chosen_input = np.concatenate((chosen_zero,chosen_one,chosen_two),axis=0)
  data_for_repair = (chosen_input-128)/128
  data_for_repair = scaler.transform(data_for_repair.reshape(batch,-1))
